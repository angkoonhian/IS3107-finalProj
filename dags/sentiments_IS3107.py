...
def get_sentimental_analysis():
    ...
    for i in range(1,6):
        res_text = session.get(f'https://sginvestors.io/news/company-announcement/latest/{i}').text
    ...
    inside_str = f"values(TO_DATE('{today}', 'YYYY-MM-DD'), " + ', '.join(sentiment_string_series) + ')'
...
with DAG(
    ...
    #     task_id='get_sentimental_analysis', python_callable=get_sentimental_analysis)
