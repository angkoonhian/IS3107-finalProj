...
def stocks_table_function(**kwargs):
    ...
    def get_key_stats(tgt_website):
        # deprecated df_list = pd.read_html(tgt_website)
        ## have to pretend to be a browser
        req = Request(tgt_website, headers={'User-Agent': 'Mozilla/5.0'})    
        df_list = pd.read_html(urlopen(req))
    ...
    stats = stats[['Market cap (intra-day)', 'Enterprise value', 'Trailing P/E',
                    'Forward P/E', 'PEG Ratio (5 yr expected)', 'Price/sales (ttm)',
                    'Price/book (mrq)', 'Enterprise value/revenue',
                    'Enterprise value/EBITDA', 

                    'Beta (5Y monthly)', '52-week change 3',
    ...
    stocks_table_data.to_csv('/Users/yuting/airflow/stocks_table_data.csv', index = False)
    print('Completed')
